...
class ClusterPredict(tf.keras.layers.Layer):

  def __init__(self, numClass, **kwargs):
    out_dim = numClass
    self.linear_layers = []
    if FLAGS.proj_head_mode == 'none':
      pass  # directly use the output hiddens as hiddens
      ...
      raise ValueError('Unknown head projection mode {}'.format(
          FLAGS.proj_head_mode))
    super(ClusterPredict, self).__init__(**kwargs)
  ...
  def call(self, inputs, training):
    ...
    # The first element is the output of the projection head.
    # The second element is the input of the finetune head.
    cluster_pred = tf.identity(hiddens_list[-1], 'cluster_pred')
    return cluster_pred#, hiddens_list[FLAGS.ft_proj_selector]
...
class Model(tf.keras.models.Model):
  ...
  def __init__(self, num_classes, **kwargs):
    ...
    self._cluster_pred = ClusterPredict(num_classes)
  ...
  def __call__(self, inputs, training):
    ...
    cluster_pred = self._cluster_pred(
    ...
    if FLAGS.train_mode == 'finetune':
      ...
      return None, supervised_head_outputs, cluster_pred
      ...
      return projection_head_outputs, cluster_pred, cluster_pred
    else:
      return projection_head_outputs, None, cluster_pred
